...
class Harimanga :
    ...
    override fun latestUpdatesRequest(page: Int) = GET("$baseUrl/home?m_orderby=latest${if (page > 1) "&page=$page" else ""}", headers)

    override fun searchMangaRequest(page: Int, query: String, filters: FilterList) = GET("$baseUrl/home?s=${query.trim()}&post_type=wp-manga${if (page > 1) "&page=$page" else ""}", headers)
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        val data = jsonParser.decodeFromString<ChapterListResponse>(response.body.string())
